    ...
    (function () {
      ...
      function buildLaneModeChart(songKey, timingOverride = null) {
        ...
        grouped.forEach((group, groupIndex) => {
          ...
          const shouldDouble = ranked.length >= 2 && (stallionBoost ? groupIndex % 3 !== 1 : groupIndex % 2 === 0);
      ...
      function sanitizePlayableChart(chart, mode = "freeform") {
        ...
        sorted.forEach((note) => {
          const active = kept.filter((entry) => entry.timeMs <= note.timeMs && note.timeMs < noteEndTime(entry));
