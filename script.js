    ...
    (function () {
      ...
      async function loadSongChart(songKey, mode = "freeform") {
        ...
        if (mode === "lanes" && window.location.protocol !== "file:") {
          const xmlText = await fetch(song.chartSrc).then((response) => {
            if (!response.ok) throw new Error("Unable to load chart");
            return response.text();
          });
          const melodyTiming = extractLeadMelodyFromMusicXml(xmlText, song);
          const laneChart = buildLaneModeChart(songKey, melodyTiming);
          musicChartCache.set(cacheKey, laneChart);
          return laneChart;
        }
        if (window.location.protocol === "file:") {
          if (mode === "lanes") {
            const fallback = buildLaneModeChart(songKey);
            musicChartCache.set(cacheKey, fallback);
            return fallback;
          }
      ...
      function handleLanePointerDown(state, event) {
        ...
        const lane = getLaneForClientX(state, event.clientX);
      ...
      function updateMusicGame(state) {
        ...
        if (state.mode === "lanes") {
          state.chart.forEach((note) => {
            ...
            if (note.type === "hold") {
              if (pointer.lane !== note.lane) {
                missLaneNote(state, note, IS_ZH ? "偏离轨道" : "Left lane");
                return;
              }
              if (trackTime >= note.endTimeMs - 22) {
                const delta = Math.abs(trackTime - note.endTimeMs);
                completeLaneNote(state, note, delta <= 70 ? 170 : 130, laneLabel(IS_ZH ? "长按" : "Hold", delta <= 70));
              }
              return;
            }
            ...
            if (note.type === "drag") {
              ...
              if (Math.abs(pointer.lane - expectedLane) > 0.85) {
                missLaneNote(state, note, IS_ZH ? "拖拽偏移" : "Drag lost");
                return;
              }
              if (trackTime >= note.endTimeMs - 22) {
                const delta = Math.abs(trackTime - note.endTimeMs);
                completeLaneNote(state, note, delta <= 70 ? 190 : 150, laneLabel(IS_ZH ? "拖拽" : "Drag", delta <= 70));
              }
            }
          });
        } else {
      ...
      function buildMusicGameUi() {
        ...
        loadSongChart("fiu", "freeform").catch(() => {});
        loadSongChart("fiu", "lanes").catch(() => {});
