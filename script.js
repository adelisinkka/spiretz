    ...
    (function () {
      ...
      function buildLaneModeChart(songKey, timingOverride = null) {
        ...
        function blockedByActiveDrag(lane, timeMs) {
          return activeLongNotesAt(timeMs).some((note) => {
            if (note.type !== "drag") return false;
            const dragHand = getLaneDragHand(note, notes);
            const span = getLaneNoteSpan(note);
            if (dragHand === "left" && lane < span.min) return true;
            if (dragHand === "right" && lane > span.max) return true;
            return false;
          });
        }
        ...
        function noteTypeFor(timeMs, holdMs) {
          if (stallionBoost && holdMs >= 620) return emittedIndex % 4 === 1 ? "drag" : "hold";
          if (stallionBoost && emittedIndex % 8 === 3 && timeMs > 8000) return "flick";
        ...
        grouped.forEach((group, groupIndex) => {
          ...
          const shouldDouble = activeLongCount === 0 && ranked.length >= 2 && (stallionBoost ? groupIndex % 3 !== 1 : groupIndex % 2 === 0);
          ...
          const available = availableLanesAt(simultaneousTime).filter((lane) => !blockedByActiveDrag(lane, simultaneousTime));
          ...
          const secondAvailable = availableLanesAt(simultaneousTime).filter((lane) => lane !== firstLane && !blockedByActiveDrag(lane, simultaneousTime));
        ...
        grouped.slice(1).forEach((group, index) => {
          ...
          if (currentTime - previousTime < (stallionBoost ? 820 : 1100)) return;
          const bridge = melody.find((entry) => entry[0] > previousTime + (stallionBoost ? 200 : 320) && entry[0] < currentTime - (stallionBoost ? 200 : 320) && entry[1] <= 700);
          if (!bridge) return;
          if (remainingCapacityAt(bridge[0]) < 1) return;
          const available = availableLanesAt(bridge[0]).filter((lane) => !blockedByActiveDrag(lane, bridge[0]));
      ...
      function buildHardLaneModeChart(songKey, laneCount = LANE_MODE_LANES, mode = "lanes") {
        ...
        function pickLane(timeMs, preferred, avoid = [], requiredSide = "") {
          let available = availableLanesAt(timeMs).filter((lane) => !avoid.includes(lane) && !blockedByActiveDrag(lane, timeMs));
          if (requiredSide) {
      ...
      function sanitizePlayableChart(chart, mode = "freeform") {
        ...
        const sorted = chart.slice().sort((a, b) => (a.timeMs - b.timeMs) || ((a.lane ?? 0) - (b.lane ?? 0)));
        const kept = [];
