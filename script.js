    ...
    (function () {
      ...
      const GRAMMAR_TEMPLATE_POOL = TEMPLATE_POOL.filter((fn) => {
        const src = String(fn);
        return /tense|aspect|mood|negation|topic marker|direct object|Genitive|Plural noun|Adjectives must|Nouns must|Verbs \(with no conjugation\)|dative|subject suffix|Reflexive|Human direct object|Royal direct object|Yes–no|article|noun phrase|word order|Imperative|conjunction|subclause|prefix|pronoun|class|possessor|linker|animate|inanimate|witnessed|hearsay|ng/.test(src);
      });
      ...
      function renderQuiz(count) {
        ...
        const allSources = TEMPLATE_POOL.concat(grammarSources);
        const grammarTarget = Math.max(1, Math.ceil(n * 0.6));
        ...
        attempts = 0;
        while (currentQuestions.length < n && attempts < 300) {
          attempts++;
          const source = randPick(allSources);
          const q = typeof source === "function" ? source() : source;
          const key = q.concept || q.signature || q.id;
          if (used.has(key)) continue;
          used.add(key);
...
(function () {
  ...
  randomBirthBtn.addEventListener("click", randomBirthday);
    ...
    // ===== Resources: Spiretz Lexicon (Nouns) =====
    (function () {
      ...
      NOUNS.forEach((entry, idx) => {
        ...
        tr.innerHTML = `<td>${idx + 1}</td><td><b>${entry.sp}</b></td><td>${entry.meaning}</td><td>${entry.pronunciation}</td><td>${entry.syllables}</td>`;
