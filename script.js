    ...
    (function () {
      ...
      function playHitSound(state, note = null, judgment = "great") {
        ...
        if (isBonus) {
          pulse("sine", judgment === "perfect" ? 740 : 680, 0.08, 0.04, 660);
          pulse("sine", judgment === "perfect" ? 988 : 932, 0.11, 0.035, 820, now + 0.028);
          return;
        }

        if (kind === "drag" || kind === "hold") {
          pulse("triangle", 220, 0.11, 0.09, 104);
          return;
        }

        if (kind === "flick") {
          pulse("triangle", 520, 0.05, 0.035, 860);
          pulse("sine", 980, 0.045, 0.02, 620);
          return;
        }

        pulse("sine", judgment === "perfect" ? 148 : 132, 0.12, 0.12, 58);
      }
      ...
      async function startMusicSong(songKey, mode = "freeform", difficulty = "normal") {
        ...
        if (state.levelLabel) {
          state.levelLabel.textContent = mode === "lanes"
            ? (difficulty === "hard" ? (IS_ZH ? "困难" : "Hard") : (IS_ZH ? "普通" : "Normal"))
            : "-";
        }
      ...
      function buildMusicGameUi() {
        ...
        shell.innerHTML = `
          <div class="music-game-header">
            ...
            <div class="music-game-level-list">
        ...
        const modeList = shell.querySelector(".music-game-mode-list");
        const levelList = shell.querySelector(".music-game-level-list");
        ...
        function syncLevelUi() {
          ...
          if (musicGameState.selectedMode === "lanes") {
        ...
        modeList.addEventListener("click", (event) => {
          ...
          syncModeUi();
          syncLevelUi();
          musicGameLayer.classList.add("is-song-list-open");
        });
        ...
        levelList.addEventListener("click", (event) => {
          ...
          musicGameState.statusLabel.textContent = IS_ZH ? "已选等级，请选歌曲" : "Level selected. Choose a song.";
          syncLevelUi();
        });
        ...
        songList.addEventListener("click", (event) => {
          ...
          if (musicGameState.selectedMode === "lanes" && !musicGameState.selectedLevel) {
            musicGameState.statusLabel.textContent = IS_ZH ? "请先选择等级" : "Select a level first";
            return;
          }
          musicGameLayer.classList.remove("is-song-list-open");
          startMusicSong(
            target.getAttribute("data-song"),
            musicGameState.selectedMode,
            musicGameState.selectedMode === "lanes" ? musicGameState.selectedLevel : "normal"
          );
        });
