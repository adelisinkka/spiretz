...
(function () {
  ...
  randomBirthBtn.addEventListener("click", randomBirthday);
    ...
    (function () {
      ...
      if (verbTbody) {
        const sortedVerbs = VERBS.slice().sort((a, b) => compareSpiretzWords(a[0], b[0]));
        sortedVerbs.forEach(([sp, en], idx) => {
          const meaning = IS_ZH ? (meaningMap[en] || en) : en;
          const meta = deriveVerbLexiconEntry(sp);
      ...
      if (nounTbody) {
        const sortedNouns = NOUNS.slice().sort((a, b) => compareSpiretzWords(a.sp, b.sp));
      ...
      if (adjectiveTbody) {
        const sortedAdjectives = ADJECTIVES.slice().sort((a, b) => compareSpiretzWords(a[0], b[0]));
      ...
      if (adverbTbody) {
        const sortedAdverbs = ADVERBS.slice().sort((a, b) => compareSpiretzWords(a[0], b[0]));
      ...
      if (prepositionTbody) {
        const sortedPrepositions = PREPOSITIONS.slice().sort((a, b) => compareSpiretzWords(a[0], b[0]));
      ...
      if (conjunctionTbody) {
        const sortedConjunctions = CONJUNCTIONS.slice().sort((a, b) => compareSpiretzWords(a[0], b[0]));
      ...
      if (questionTbody) {
        const sortedQuestions = QUESTIONS.slice().sort((a, b) => compareSpiretzWords(a[0], b[0]));
